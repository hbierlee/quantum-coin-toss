...
\only<1>{1. Alice encodes a random bit-string with a randomly chosen base and sends them to Bob}
\only<2>{2. Bob randomly chooses a base for each photon and fills corresponding measurement table and makes a guess}
\only<3>{3. Alice reveals the correct base and sends original bit-string over classic communication channel}
